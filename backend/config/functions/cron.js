...
module.exports = {
  ...
  //
  // }
  // Run once a day at eight in the morning
  "0 8 * * *": async () => {
    // Search for subscriptions where the next delivery is today
      ...
      subscriptions.map(async (subscription) => {
        ...
        try {
          ...
          var order = await strapi.services.order.create({
            ...
            total: Math.round(subscription.variant.price * 1.21),
            tax: Math.round(subscription.variant.price * 0.21),
            items: [
              {
